...
export const getMMTFlightData = async (
  ...
  try {
    ...
    const fromCityText = await fromCity.evaluate((el) =>
      el.querySelector("#fromCity").getAttribute("value"),
    );
    console.log(
      `From : ${fromCityText} , ${fromCityText === placeFrom ? "Success" : "Failed"}`,
    );
    ...
    for (let i = 0; i < 3; i++) {
      ...
      await waitForProcessing(PROCESSING_WAIT * (i + 1));
      await page.waitForSelector("#react-autowhatever-1-section-0-item-0");
      await page.click("#react-autowhatever-1-section-0-item-0");

      // Test if the action is successful
      const toCityText = await toCity.evaluate((el) =>
        el.querySelector("#toCity").getAttribute("value"),
      );
      console.log(
        `To : ${toCityText} , ${toCityText === placeTo ? "Success" : "Failed"}`,
      );
    ...
    for (const month of months) {
      const monthYear = await month.evaluate((el) =>
        el
          .querySelector("div.DayPicker-Caption")
          .textContent.split(" ")
          .join(""),
      );
      if (monthYear === [dateMonth, dateYear].join("")) {
        ...
        for (const day of days) {
          const [dayText, cost] = await day.evaluate((el) =>
            el.textContent.split("₹"),
          );
          const isDisabled = await day.evaluate((el) =>
            el.classList.contains("DayPicker-Day--disabled"),
          );
    ...
    await waitForProcessing();
    const [cdateDay, cdateMonth, cdateYear] = await dateData[0].evaluate(
      (el) => {
        let result = [];
    ...
    console.log(
      `Date : ${cdateDay} ${cdateMonth} ${cdateYear} , ${cdateDay === dateDay && dateMonth.includes(cdateMonth) && dateYear.includes(cdateYear) ? "Success" : "Failed"}`,
    );
    ...
    for (const flight of flights) {
      ...
      const flightDetails = await flight.evaluate((el) => {
        ...
        const flightTime = el.querySelector(
          ".timeInfoRight .flightTimeInfo > span",
        ).textContent;
        ...
        const flightPrice = el
          .querySelector("div.priceSection > div > div > div")
          .textContent.split("p")[0]
          .split(" ")[1];
